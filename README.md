...
| ComfyUI + Chatterbox 노드 | 음성 생성 | 로컬 GPU면 무료 |

음성은 로컬 ComfyUI 대신 클라우드 ComfyUI로도 됩니다. `voice.json`의 `host`만 바꾸면 됩니다.
...
#   host를 자기 ComfyUI 주소로, 참조 음성 파일을 준비 (docs/RECORDING.md)
