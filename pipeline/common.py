...
NAR_LUFS = -16    # 씬 단위 내레이션. **모노 파일 기준**이다 — 40 이 -ac 1 로 만든다.
                  # 마스터는 스테레오라 이 소리가 두 채널에 복제되면서 BS.1770 이
                  # 채널 파워를 더해 **-13 으로 잡힌다**(+3.01). 클립 소재는 원래
                  # 스테레오라 그대로다. 두 값을 나란히 비교할 때 이 3 dB 을 빼먹으면
                  # 안 된다 — E01 에서 「인트로와 2 LU 차이」로 정한 것이 실제로는
                  # 5 LU 였다. 값을 적을 때 **어느 기준인지 같이 적는다.**(음악 감독)
