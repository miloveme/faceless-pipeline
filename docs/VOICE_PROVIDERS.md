...
**주소는 사람마다 다르고 저장소에 없습니다.** `voice.json` 은 개인 자산이라 커밋되지 않으므로,
위 `192.0.2.10` 도 `127.0.0.1` 도 예시일 뿐입니다. 서버가 다른 기계에 있으면 `127.0.0.1` 로는 안 붙습니다.
**모르면 추측하거나 포트를 훑지 말고 사용자에게 물어보세요.** 이 값을 고치는 것은 엔지니어 일입니다
(목소리 값 — 제공자·참조 음성·파라미터 — 은 음악 감독 것입니다 → [AGENTS.md](AGENTS.md)).
